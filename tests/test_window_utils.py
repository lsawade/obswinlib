...
from lwsspy.utils.io import load_json
...
def test_sort_windows_on_channel_and_location():
    sta_win = {"II.AAK.10.BHZ": [1, 2, 3], "II.AAK.10.BHR": [1, 2],
               ...
               "II.AAK.10.EHZ": [1, 2],
               "II.AAK..LHZ": [1, 2, 3, 4]}
    results = owl.sort_windows_on_channel_and_location(sta_win)
    _true = {
        'BH': {
...
def test_pick_location_with_more_windows():
    ...
    choosen = owl.pick_location_with_more_windows(input)
...
def test_merge_instruments_windows():
    ...
    choosen_wins = owl.merge_instruments_window(sta_win)
    _true = {
...
def test_sort_windows_on_channel():
    sta_win = {"II.AAK.10.BHZ": [1, 2, 3], "II.AAK.10.BHR": [1, 2],
               ...
               "II.AAK.10.EHZ": [1, 2],
               "II.AAK..LHZ": [1, 2, 3, 4]}
    sort_dict = owl.sort_windows_on_channel(sta_win)
    _true = {
        'BH': {'nwins': 13, 'traces': ['II.AAK.10.BHZ', 'II.AAK.10.BHR',
                                       'II.AAK.10.BHT', 'II.AAK.00.BHZ',
                                       'II.AAK..BHZ']},
        'EH': {'nwins': 14, 'traces': ['II.AAK..EHZ', 'II.AAK..EHR',
                                       'II.AAK.10.EHZ']},
...
def test_pick_channel_with_more_windows():
    ...
    results = owl.pick_channel_with_more_windows(input)
    assert results == "BH"
...
def test_merge_channels_window():
    ...
    results = owl.merge_channels_window(sta_win)
...
def test_merge_station_windows():
    ...
    results = owl.merge_station_windows(sta_win)
    _true = {"II.AAK..EHZ": [1, 2, 3, 4, 5, 6, 7, 8],
             "II.AAK..EHR": [1, 2, 3, 4]}
    assert results == _true
...
def test_merge_windows():
    ...
    results = owl.merge_windows(windows)
...
def test_generate_log_content():
    windows = {"II.BBK": {"II.BBK..BHZ": [1, 2, 3, 4], "II.BBK..BHR": [],
               ...
               "II.AAK": {"II.AAK.10.BHZ": [1, 2, 3], "II.AAK.10.BHR": [1, 2],
                          "II.AAK.10.BHT": [1]},
               "II.CCK": {}}
    log = owl.generate_log_content(windows)
    _true = {
...
def test_stats_all_windows(tmpdir):
    ...
    owl.stats_all_windows(windows, "proc_obsd_17_40", "proc_synt_17_40",
                          True, outputfile)
