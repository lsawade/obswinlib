...
TESTBASE_DIR = os.path.dirname(os.path.abspath(__file__))
...
def test_update_user_levels_raise():
    user_module = "lwsspy.seismo.window.tests.which_does_not_make_sense"
    with pytest.raises(Exception) as errmsg:
        owl.update_user_levels(user_module, None, None, None,
                               None, None)

    assert "Could not import the user_function module" in str(errmsg)

    user_module = "lwsspy.seismo.window.io"
    with pytest.raises(Exception) as errmsg:
        owl.update_user_levels(user_module, None, None, None,
                               None, None)
...
def test_window_on_stream():
    ...
    config_file = os.path.join(DATA_DIR, "window", "27_60.BHZ.config.yaml")
    config = owl.read_yaml(config_file)
    config_dict = dict(
    ...
    windows = owl.window_on_stream(obs_tr, syn_tr, config_dict, station=inv,
                                   event=cat, _verbose=False,
                                   figure_mode=False)
...
def test_window_on_stream_user_levels():
    ...
    _mod = "lwsspy.seismo.window.tests.user_module_example"
...
def test_plot_window_figure(tmpdir):
    ...
    owl.plot_window_figure(str(tmpdir), obs_tr.id, ws, True,
                           figure_format="png")
